...
job_id="${job_id:-0}"
hostfile=$job_root/$job_set/cluster_j$job_id
PY=/usr/bin/python
...
$MPIPATH/bin/mpirun --prefix $MPIPATH -hostfile $hostfile -bind-to none -map-by slot --allow-run-as-root \
    ...
    $PY horovod_trainer.py --job-root $job_root --job-set $job_set --job-id $job_id --mode real --global-sync $(date +%Y-%m-%d-%H-%M-%S)
